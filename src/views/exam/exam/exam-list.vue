<template>
  ...
  <div>
    ...
    >
      ...
      <el-table-column prop="examId" label="考试编号" min-width="200" align="center" />
      <el-table-column prop="examName" label="考试名称" min-width="200" align="center" show-overflow-tooltip />
      ...
      <el-table-column prop="isLimitTime" label="是否限时" min-width="100" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.isLimitTime === 1" type="danger">限时</el-tag>
    ...
    <!-- 添加弹窗 -->
    <el-dialog title="添加考试" :visible.sync="addDialog" width="50%">
      <exam-add @closeAddDialog="closeAddDialog" @getByPage="getByPage" />
    </el-dialog>
    <!--
      修改弹窗
      :exam="exam" 用于传递参数对象
    -->
    <el-dialog title="修改考试" :visible.sync="updateDialog" width="50%">
      <exam-update :exam="exam" @closeUpdateDialog="closeUpdateDialog" @getByPage="getByPage" />
    </el-dialog>
...
// 导入组件
import ExamAdd from './exam-add.vue'
import ExamUpdate from './exam-update.vue'
export default {
  //  定义添加的组件 子组件/私有组件
  components: {
    ExamAdd,
  ...
  methods: {
    ...
    changeExpand(e) {
      console.log(e)
      examApi.get(e.examId)
    },
    // 数据表格的多选择框选择时触发
    handleSelectionChange(selection) {
      this.selectexams = selection.map(item => item.examId)
    ...
    toDelete(row) {
      ...
      this.$confirm('是否删除?', '提示', {
        ...
        examApi.delete(this.ids).then(res => {
          this.$message.success(res.msg)
          this.getByPage()
        })
      }).catch(() => {
