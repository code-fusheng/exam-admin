  ...
  <div>
    ...
    <el-form :inline="true" :model="page" class="demo-form-inline" size="mini">
      ...
      <el-form-item label="起始日期">
        <el-date-picker
          v-model="page.params.repositoryTime"
          type="daterange"
          align="right"
          unlink-panels
          range-separator="至"
    ...
    >
      ...
      <el-table-column label="操作" min-width="300" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="mini" icon="el-icon-view" @click="toRead(scope.row.repositoryId)">查看</el-button>
          <el-button v-if="scope.row.isEnabled === 0" type="text" icon="el-icon-check" size="mini" @click="toEnable(scope.row.repositoryId)">启用</el-button>
          <el-button v-if="scope.row.isEnabled === 1" type="text" icon="el-icon-close" size="mini" @click="toDisable(scope.row.repositoryId)">弃用</el-button>
    ...
    <!-- 添加弹窗 -->
    <el-dialog title="添加题库" :visible.sync="addDialog">
      <repository-add @closeAddDialog="closeAddDialog" @getByPage="getByPage" />
    </el-dialog>
    <!--
      修改弹窗
      :repository="repository" 用于传递参数对象
    -->
    <el-dialog title="修改题库" :visible.sync="updateDialog">
      <repository-update :repository="repository" @closeUpdateDialog="closeUpdateDialog" @getByPage="getByPage" />
    </el-dialog>
...
<script>
// 导入api接口定义的方法 接收变量为 repositoryApi
...
export default {
  ...
  methods: {
    ...
    getByPage() {
      this.loading = true
      this.page.list = []
      repositoryApi.getByPage(this.page).then(res => {
        this.page = res.data
        this.loading = false
        console.log(res)
      })
    ...
    toEnable(id) {
      this.$confirm('是否启用？', '提示', {
        ...
        repositoryApi.enable(id).then(res => {
    ...
    toDisable(id) {
      this.$confirm('是否弃用?', '提示', {
        ...
        repositoryApi.disable(id).then(res => {
          this.$message.success(res.msg)
          this.getByPage()
        })
      }).catch(() => {
    ...
    toDelete(row) {
      ...
      this.$confirm('是否删除?', '提示', {
        ...
        repositoryApi.delete(this.ids).then(res => {
          this.$message.success(res.msg)
          this.getByPage()
        })
      }).catch(() => {
    ...
    refresh() {
      this.page.currentPage = 1
      this.page.params.repositoryName = null
      this.page.params.repositoryTime = null
