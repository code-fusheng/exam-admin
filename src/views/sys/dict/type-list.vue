<template>
  ...
  <div>
    ...
    >
      ...
      <el-table-column label="字典类型" prop="dictType" min-width="180" align="center" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <router-link :to="'dictData/' + scope.row.dictType" class="link-type">
      ...
      <el-table-column label="操作" min-width="300" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="mini" icon="el-icon-view" @click="toRead(scope.row.dictId)">查看</el-button>
    ...
    <!-- 添加弹窗 -->
    <el-dialog title="添加字典类型" :visible.sync="addDialog">
    ...
    <!--
      ...
      :type="type" 用于传递参数对象
    -->
    <el-dialog title="修改字典类型" :visible.sync="updateDialog">
...
<script>
// 导入api接口定义的方法 接收变量为 typeApi
import typeApi from '@/api/sys/dict/type.js'
// 导入组件
  ...
  methods: {
    ...
    toDelete(row) {
      ...
      this.$confirm('是否删除?', '提示', {
        ...
        typeApi.delete(this.ids).then(res => {
          this.$message.success(res.msg)
          this.getByPage()
        })
      }).catch(() => {
    ...
    refresh() {
      this.page.currentPage = 1
      this.page.params.dictName = null
      this.page.params.dictType = null
      this.page.params.dictTypeTime = null
